...
class Vimeo90KDataset(data.Dataset):
    ...
    def __init__(self, opts_dict, radius):
        ...
        # dataset paths
        self.gt_root = op.join('data/vimeo90k/', self.opts_dict['gt_path'])
        self.lq_root = op.join('data/vimeo90k/', self.opts_dict['lq_path'])

        # extract keys from meta_info.txt
        self.meta_info_path = op.join(self.gt_root, 'meta_info.txt')
...
class VideoTestVimeo90KDataset(data.Dataset):
    ...
    def __init__(self, opts_dict, radius):
        ...
        self.gt_root = op.join('data/vimeo90k/', self.opts_dict['gt_path'])
        self.lq_root = op.join('data/vimeo90k/', self.opts_dict['lq_path'])
        self.meta_info_path = op.join('data/vimeo90k/', self.opts_dict['meta_path'])
        ...
        while True:
            ...
            gt_path = op.join(self.gt_root, vid_name + '.yuv')
