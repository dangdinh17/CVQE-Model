"""Create LMDB only for training set of LDV2.
GT: non-overlapping 7-frame sequences extracted from 240 videos.
LQ: HM16.5-compressed sequences.
key: assigned from 0000 to 9999.
NOTICE: MAX NFS OF LQ IS 300!!!
Sym-link LDV2 dataset root to ./data folder."""
import os
import glob
import yaml
import argparse
...
def create_lmdb():
    ...
    lq_video_list = [op.join(lq_dir, gt_video_path.split('/')[-1]) for gt_video_path in gt_video_list] #用低质量视频的目录路径替换了原始路径中的高质量视频目录路径，从而得到了低质量视频的路径列表
    ...
    for gt_video_path in gt_video_list:
        ...
        frm_list.append([list(range(iter_seq * (2 * radius+1), (iter_seq + 1) * (2 * radius+1))) for iter_seq in range(num_seq)])
    ...
    for iter_vid in range(len(gt_video_list)):
        ...
        for iter_frm in range(len(frms)):
            key_list.extend(['{:03d}/{:03d}/im{:d}.png'.format(iter_vid + 1, iter_frm + 1, i) for i in range(1, (2 * radius+1) + 1)])
    ...
    # sym-link 创建链接
    if not op.exists('data/LDV2'):
        if not op.exists('data/'):
            os.system("mkdir data/")
        os.system(f"ln -s {root_dir} ./data/LDV2")
        print("Sym-linking done.")
    else:
        print("data/LDV2 already exists.")
